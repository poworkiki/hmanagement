...
- [x] T037 [P] [US1] [VALID] (SC-005) `docker restart` de toute la stack — **non testé explicitement**, mais recreate Coolify complet a remonté 12 containers en ~3 min → signal indirect OK. ✅ validé indirectement 2026-04-23.
...
- [x] T050 [US2] Env vars OIDC injectées via Coolify API (script orchestrateur) : 6 vars `GOTRUE_EXTERNAL_KEYCLOAK_*` + `GOTRUE_EXTERNAL_EMAIL_ENABLED=false` pushées + reconfig.
- [x] T051 [US2] Restart orchestré (stop → start via Coolify API). Coolify a bloqué les containers en état `Created` → forcé start manuel `docker start` sur les 13 containers. Après 2-3 min, stack healthy. **Leçon runbook** : les restarts Coolify sur stack multi-container Supabase peuvent nécessiter un rescue manuel. Documenté.
- [x] T052 [US2] Côté Authentik (`auth.hma.business`) : ajouter le compte cible au groupe `supabase-hma-admins` (créé en T016). ✅ 2026-04-23 — `hmadmin` créé et assigné au groupe via API Authentik (T017.1).
- [x] T053 [US2] Sur `https://supabase.hma.business/` → login via OIDC Authentik → challenge TOTP → session Supabase authentifiée. ✅ 2026-04-23 — flow validé en Phase 4 avec `hmadmin` + Microsoft Authenticator (commit `61b4bb0`).
- [~] T054 [US2] [VALID] (FR-007) Vérifier dans les claims JWT la présence d'un attribut attestant MFA (`amr: ["mfa"]` ou équivalent). **Déféré post-merge** : nice-to-have ops-routine, enforcement MFA déjà vérifié côté Authentik (stage `default-authentication-mfa-validation` active globalement, cf. T016).
- [~] T055 [P] [US2] [VALID] (FR-008) Session admin inactive 60 min → redirection login. **Déféré post-merge** : time-gated (60 min d'attente), `GOTRUE_JWT_EXP=3600` est bien configuré côté env var (vérifiable sans test).
- [~] T056 [P] [US2] [VALID] (FR-009) Authentik Magic Link TTL ≤ 15 min. **Déféré post-merge** : Authentik en MVP utilise OIDC direct, pas le flow Magic Link. Le TTL effectif est celui du token OIDC standard (15min défaut Authentik).
- [~] T057 [P] [US2] [VALID] Retirer compte du groupe → refus login. **Déféré post-merge** : nécessite un 2e compte pour ne pas casser le seul user actif `hmadmin` pendant le test.
- [~] T058 [P] [US2] [VALID] (SC-002) Chronométrer activation 2e admin end-to-end < 5 min. **Déféré post-merge** : nécessite création d'un 2e user humain (pas de use-case MVP, `hmadmin` seul pour l'instant).
...
- [~] T081 [P] [US3] [VALID] (SC-003, FR-014) Attendre 24 h après T076 et vérifier qu'un **deuxième** snapshot automatique est apparu sur R2. **Déféré post-merge** : time-gated (cron actif `/etc/cron.d/supabase-backup`, 1ère exécution auto prévue 2026-04-24 03h30 UTC). Vérification trivial via `restic snapshots` le lendemain.
- [x] T082 [P] [US3] [VALID] (SC-004, FR-017) Le drill mensuel T078 valide déjà le critère : restauration < 30 min, intégrité OK. ✅ 2026-04-23 — T078 a run en 9s (budget 30min) avec 11 schemas + 174 relations restaurées, exit 0. Critère validé.
- [~] T083 [P] [US3] [VALID] (FR-018) Simuler panne `pg-backup.sh` → notif Telegram échec < 15 min. **Déféré post-merge** : le script envoie bien sur échec (code audité dans `infra/supabase/backups/pg-backup.sh` avec notify Telegram), mais le test "sabotage volontaire des creds R2" risque de polluer le cron actif. Sera testé au prochain changement de creds R2 (rotation annuelle) où le scénario est naturel.
...
- [~] T104 [US4] [VALID] (SC-008) Déclencher une rotation de `JWT_SECRET` (Coolify UI → env var → nouvelle valeur → restart) → confirmer que l'ancienne `SERVICE_ROLE_KEY` renvoie 401 en < 5 min. **DÉFÉRÉ hors feature 001** (décision 2026-04-23) : la procédure est intégralement documentée dans `docs/runbooks/supabase-secret-rotation.md` (T105/T145). L'exécution destructive n'apporte pas de valeur tant qu'aucun consommateur externe (app Next.js, workflows n8n, scripts CI) n'utilise les clés. Elle sera exécutée **à la 1ère rotation trimestrielle planifiée** (cadence fixée dans le runbook) où elle validera la procédure **en contexte de production réel**. SC-008 reste couverte **théoriquement** par le runbook ; la validation empirique attend un système qui a quelque chose à casser.
...
- [x] T143 Cleanup post-US4 : SSH sur VPS → `psql ... DROP TABLE public.test_contract_table;` → supprimer la table temporaire créée en T102. ✅ 2026-04-23 — droppée après 8 rows de smoke-test accumulés sur les différentes runs.
